...
LFPmax = 5e-3*max(abs([LFP_NC_saccade.*1e3 LFP_Go_saccade.*1e3]), ...
...
hold on
for ii = 1:Ne
    plot(ts_saccade, LFP_Go_saccade(Ne-ii+1,:).*1e3./LFPmax + ...
        (ze(ii)) + 500, ...
...
hold on
for ii = 1:Ne
    plot(ts_saccade,LFP_NC_saccade(Ne-ii+1,:).*1e3./LFPmax + ...
        (ze(ii)) + 500, ...
        'color', 'k', 'clipping','on', 'linewidth', 1)
end
...
% -- error - correct
subplot(1,3,3)
LFPmax = 5e-3*max(abs(LFP_NC_saccade.*1e3 - ...
    LFP_Go_saccade.*1e3),[],'all'); % scale for the LFP plot
hold on
for ii = 1:Ne
    plot(ts_saccade,(LFP_NC_saccade(Ne-ii+1,:).*1e3 - ...
        LFP_Go_saccade(Ne-ii+1,:).*1e3)./LFPmax + ...
        (ze(ii)) + 500, ...
        'color', 'k', 'clipping','on', 'linewidth', 1)
end
ax = gca;
