%% calculate the ISI dist for the sim L5 neurons and its density maps
...
tone_times_Go = double(tone_times_Go(1,1:num_trials)).*1e-3; % sec, tone times 
% relative to saccade
% tone_times_Go = ones(size(tone_times_Go(1,1:num_trials))).*3;
...
% saccade times relative to target
tone_times_NC = double(tone_times_NC(1,1:num_trials)).*1e-3; % sec, tone times
% relative to saccade
% tone_times_NC = ones(size(tone_times_NC(1,1:num_trials))).*3;
...
for n=0:(num_neurons-1)
    ...
    for t=1:num_trials
        ...
        soma_spkTimesT_tone = soma_spkTimesT(soma_spkTimesT < saccade_times_Go(1,t) ...
            + tone_times_Go(1,t));
...
for n=0:(num_neurons-1)
    ...
    for t=1:num_trials
        ...
        soma_spkTimesT_tone = soma_spkTimesT(soma_spkTimesT < saccade_times_NC(1,t) + ...
            tone_times_NC(1,t));
...
for ii=1:num_neurons
    figure('Visible', 'off');
    cfg.interpolate = 5;
    cfg.spikechannel = spikes_Go_saccade.label{ii};
    cfg.window = 'gausswin';
    cfg.winlen = 0.005;
    cfg.scatter = 'no';
...
save(fullfile('data', 'sim_data','L5_isi_dist_sim.mat'), ...
...
save(fullfile('data', 'sim_data','L5_spk_tone_ft_struct_sim.mat'), ...
