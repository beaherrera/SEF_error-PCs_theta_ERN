...
#define factor_GABAB1 _p[10]
#define DA_GABAB _p[11]
#define DB_GABAB _p[12]
#define v _p[13]
#define _g _p[14]
#define _tsav _p[15]
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 2;
 	...
 	ivoc_help("help ?1 GABAB C:/Users/bherr035/OneDrive - Florida International University/!!PhD_Research/Python-files/Theta_paper_sim_mpi_implementation/cell_models/EyalEtAl2018/ActiveMechanisms/GABAB.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   ...
   DB_GABAB = ( - B_GABAB + factor_GABAB1 * A_GABAB ) / tau_d_GABAB ;
   }
 return _reset;
}
 ...
 static int state (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
    ...
    B_GABAB = B_GABAB + (1. - exp(dt*(( ( - 1.0 ) ) / tau_d_GABAB)))*(- ( ( ( ( factor_GABAB1 )*( A_GABAB ) ) ) / tau_d_GABAB ) / ( ( ( - 1.0 ) ) / tau_d_GABAB ) - B_GABAB) ;
   }
  return 0;
}
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
 ...
 _tsav = t; {
   _args[1] = _args[0] ;
    ...
    double __primary = (A_GABAB + _args[1]) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau_r_GABAB ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau_r_GABAB ) - __primary );
    A_GABAB += __primary;
  } else {
 A_GABAB = A_GABAB + _args[1] ;
     }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = B_GABAB;
    double __primary = (B_GABAB + _args[1]) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( ( - 1.0 ) ) / tau_d_GABAB ) ) )*( - ( ( ( ( factor_GABAB1 )*( A_GABAB ) ) ) / tau_d_GABAB ) / ( ( ( - 1.0 ) ) / tau_d_GABAB ) - __primary );
    B_GABAB += __primary;
  } else {
 B_GABAB = B_GABAB + _args[1] ;
     }
 } }
...
static void initmodel(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
  ...
  B_GABAB = B_GABAB0;
 {
   double _ltp_GABAB , _ltemp_GABAB ;
 A_GABAB = 0.0 ;
   B_GABAB = 0.0 ;
   _ltemp_GABAB = tau_r_GABAB / tau_d_GABAB ;
   factor_GABAB1 = pow( _ltemp_GABAB , ( 1.0 / ( _ltemp_GABAB - 1.0 ) ) ) ;
